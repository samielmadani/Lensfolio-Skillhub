...
public class GroupControllerTest {
    ...
    @Test
    public void getGroupIdsTest() throws Exception{
        PaginatedGroupsResponse groups = groupClientGRPCService.getGroups();
        List<GroupDetailsResponse> groupsList = groups.getGroupsList();

        Mockito.when(groupsList.get(0).getGroupId()).thenReturn(1);

        RequestBuilder request = MockMvcRequestBuilders.get("api/groups/ids")
                .accept(MediaType.ALL)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}");
    ...
    @Test
    public void groupFragmentTest() throws Exception {
        GroupDetailsResponse response = groupClientGRPCService.getGroup(anyInt());

        Mockito.when(groupClientGRPCService.getGroup(anyInt())).thenReturn(response);

        RequestBuilder request = MockMvcRequestBuilders.get("group/{id}")
                .accept(MediaType.ALL)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}");
